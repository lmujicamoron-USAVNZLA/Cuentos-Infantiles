...
function validateContent(text) {
    ...
    for (let forbidden of SAFETY_BLACKLIST) {
        ...
        if (regex.test(lowerText)) {
            // Excepción para "pelea" o "lucha" si el contexto es de superhéroes (v17.8)
            const heroContext = lowerText.includes("batman") || lowerText.includes("spiderman") || lowerText.includes("hero");
            if (heroContext && (item === 'matar' || item === 'sangre')) return { valid: false, word: item }; // Sangre y matar siguen prohibidos
            if (heroContext) continue; // Permitir otros términos en contexto heroico
